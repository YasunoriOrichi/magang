...
<body class="bg-gray-100">
  ...
  <aside id="sidebar" class="bg-gray-800 text-white w-64 space-y-6 py-7 px-2 hidden md:block min-h-screen">
    <nav class="space-y-4">
      ...
      <a href="itemCustomer.php" class="block py-2 px-4 rounded bg-gray-700 text-yellow-400">Item Customer</a>
    </nav>
  </aside>

  <!-- Main Content -->
  <div class="p-6 max-w-5xl mx-auto">
    <h1 class="text-2xl font-bold mb-6">Kelola Item Kustomer</h1>
  
    <!-- Form Tambah Item -->
    <div class="bg-gray-800 text-white rounded-xl shadow-md p-6 mb-8">
      <h2 class="text-lg font-semibold mb-4">Tambah Item Kustomer Baru</h2>
      <form action="../function/addItemCustomer.php" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label for="nama" class="block mb-1 text-sm font-medium">Nama Kustomer</label>
          <select name="nama" id="nama" class="w-full px-3 py-2 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-400">
          <option value="">
            <?php
        ...
        <div>
          <label for="item" class="block mb-1 text-sm font-medium">Nama Item</label>
          <select name="item" id="item" class="w-full px-3 py-2 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-400">
            <option value="">
            <?php
        ...
        <div>
          <label for="harga" class="block mb-1 text-sm font-medium">Harga Item</label>
          <input type="number" id="harga" name="harga" placeholder="Contoh: 15000"
            class="w-full px-3 py-2 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-400">
        </div>
        <div>
          <label for="jumlah" class="block mb-1 text-sm font-medium">Total Item</label>
          <input type="number" id="jumlah" name="jumlah" placeholder="Contoh: 10"
            class="w-full px-3 py-2 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-400">
        </div>
        <div class="flex items-end">
          <button type="submit"
            class="bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-4 py-2 rounded-md w-full md:grid-cols-2 gap-4">Tambah</button>
        </div>
      </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
      <h2 class="text-lg font-semibold mb-4 text-gray-800">Daftar Item Kustomer</h2>
      <table class="table-auto w-full text-sm text-left text-gray-700">
        <thead class="bg-gray-200">
          <tr>
            <th class="px-4 py-2">ID</th>
            <th class="px-4 py-2">No Reff</th>
            <th class="px-4 py-2">Nama Kustomer</th>
        ...
        <?php
          ...
          JOIN customer c ON ic.CUSTOMER = c.ID
          JOIN item i ON ic.ITEM = i.ID";
          ...
          if (mysqli_num_rows($result) > 0) {
              ...
              while ($row = mysqli_fetch_assoc($result)) {
                  ...
                  echo "<td class='px-4 py-2'>" . $row['itemName'] . "</td>";
                  ...
                  echo "<td>
                        <form action='../function/deleteItemCustomer.php' method='GET'>
                        <button type='submit' name='ID' value='" . $row['ID'] . "'
              ...
              echo "<tr><td colspan='9' class='text-center px-4 py-2'>Tidak ada data</td></tr>";
          }
          ?>
        </tbody>
      </table>
